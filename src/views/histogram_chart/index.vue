...
onMounted(() => {
  ...
  const rects = group
    ...
    .on("mouseover", function (e) {
      ...
      // 筛选出相同 label 的元素
      const sameLabelRects = rects.filter((d) => d.label === data.label);
      // 筛选出不同 label 的元素
      const differentLabelRects = rects.filter((d) => d.label !== data.label);
      // 保持相同 label 元素的 opacity 为 1
      sameLabelRects.style("opacity", "1");
      // 调整不同 label 元素的 opacity 为 0.2
      differentLabelRects.style("opacity", "0.2");
    })
    .on("mouseout", () => {
  ...
  const groupItem = group
    .append("rect")
    .attr("x", (d) => columWidth * ["Q1", "Q2", "Q3", "Q4"].indexOf(d.label))
    .attr("y", 0)
  ...
  group
    ...
    .attr(
      "x",
      (d) => columWidth * (["Q1", "Q2", "Q3", "Q4"].indexOf(d.label) - 1) + 10
    )
    .attr("y", function (d) {
    ...
    .attr("transform", (d, i) => {
      // 使用 transform 属性旋转文本
      const x = columWidth * (["Q1", "Q2", "Q3", "Q4"].indexOf(d.label) - 1);
      const y = chartHeight;
...
function onGroupLabel(e, d) {
  const mouseX = e.pageX - 220;
  const mouseY = e.pageY - 120;
  ...
  svg.on("mousemove", function (e) {
    labelOverlay.attr(
      "transform",
      `translate(${e.pageX - 120}, ${e.pageY - 120})`
    );
  });
}
/**
